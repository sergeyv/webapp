...
// TODO: Make those into some semi-global "helpers" defined separately
// from the view? like webapp.helpers.simple_value("Hello", world) ?

TemplatedView.prototype.simpleValue = function(title, value) {
    /*
...
TemplatedView.prototype.nameAndId = function(title, obj, root) {
    /*
     Returns a nicely-formatted link to a related object
    */
    if (obj.id) {
        return '<div><label>'+title+'</label><a href="' + root + obj.id + '">'
            + obj.name + '</a></div>';
    } else {
        return "";
    }
...
TemplatedView.prototype.emailValue = function(title, value) {
    /*
     Returns a nicely-formatted bit of html for an email
    */
    if (value) {
        return '<div><label>'+title+'</label><a href="mailto:'+value+'">'+ value + '</a></div>';
    } else {
        return "";
    }
}
